...
Frame()
{
  ...
  sesc_barrier_init(&(Global->SlaveBarrier));
;
  sesc_barrier_init(&(Global->TimeBarrier));
;
  sesc_lock_init(&(Global->IndexLock));
;
  sesc_lock_init(&(Global->CountLock));
;
  {int i;for(i=0;i<MAX_NUMPROC+1;i++){sesc_lock_init(&(Global->QLock[i]));
;}};
