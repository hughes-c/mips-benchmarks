...
VOID	StartRayTrace()
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(gm->pidlock));
#endif
	sesc_lock(&(gm->pidlock));

#ifdef SIMICS
       SIMICS_LOCKED(&(gm->pidlock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
	pid = gm->pid++;
	{
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(gm->pidlock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(gm->pidlock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  begin = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
...
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(), );
#endif
	sesc_barrier(&(), );

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&());
#endif
}
	...
	{
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(gm->start), gm->nprocs);
#endif
	sesc_barrier(&(gm->start), gm->nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(gm->start));
#endif
}
  ...
  end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
...
CHAR	*argv[];
	...
	sesc_barrier_init(&(gm->start));

	sesc_lock_init(&(gm->pidlock));

	sesc_lock_init(&(gm->ridlock));

	sesc_lock_init(&(gm->memlock));

	{int i;for(i=0;i<nprocs;i++){sesc_lock_init(&(gm->wplock[i]));
;}}
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the 
  ...
  begin = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
	...
	sesc_spawn(StartRayTrace, NULL, 0);
