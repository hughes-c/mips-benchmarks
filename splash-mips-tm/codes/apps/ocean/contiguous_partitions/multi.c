...
/* before proceeding with relaxation                               */
     ...
     {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->error_barrier), nprocs);
#endif
	sesc_barrier(&(bars->error_barrier), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->error_barrier));
#endif
}  
     copy_black(k,my_num);
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(locks->error_lock));
#endif
	sesc_lock(&(locks->error_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(locks->error_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
     ...
     }
     {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(locks->error_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(locks->error_lock));
       SIMICS_STOP_IGNORE();
#endif
