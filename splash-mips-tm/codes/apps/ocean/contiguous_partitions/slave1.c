...

{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(locks->idlock));
#endif
	sesc_lock(&(locks->idlock));

#ifdef SIMICS
       SIMICS_LOCKED(&(locks->idlock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
     ...
     global->id = global->id+1;
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(locks->idlock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(locks->idlock));
       SIMICS_STOP_IGNORE();
#endif
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_prini), nprocs);
#endif
	sesc_barrier(&(bars->sl_prini), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_prini));
#endif
}
...
/* compute psib array (one-time computation) and integrate into psibi */
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_psini), nprocs);
#endif
	sesc_barrier(&(bars->sl_psini), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_psini));
#endif
} 
...
/* compute input curl of wind stress */
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_onetime), nprocs);
#endif
	sesc_barrier(&(bars->sl_onetime), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_onetime));
#endif
}
 ...
 ***************************************************************/
  ...
  global->trackstart = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
}
         }
