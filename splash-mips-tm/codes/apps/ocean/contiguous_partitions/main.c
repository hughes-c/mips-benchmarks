...
struct bars_struct {
   ...
   sbarrier_t (sl_prini);
   sbarrier_t (sl_psini);
   sbarrier_t (sl_onetime);
   sbarrier_t (sl_phase_1);
   sbarrier_t (sl_phase_2);
   sbarrier_t (sl_phase_3);
   sbarrier_t (sl_phase_4);
   sbarrier_t (sl_phase_5);
   sbarrier_t (sl_phase_6);
   sbarrier_t (sl_phase_7);
   sbarrier_t (sl_phase_8);
   sbarrier_t (sl_phase_9);
   sbarrier_t (sl_phase_10);
   sbarrier_t (error_barrier);
} *bars;
...

{
  ...
  start = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
   ...
   imx = (int *) malloc(numlev*sizeof(int));;
   jmx = (int *) malloc(numlev*sizeof(int));;
   ...
   xpts_per_proc = (int *) malloc(numlev*sizeof(int));;
   ypts_per_proc = (int *) malloc(numlev*sizeof(int));;
   ...
   for (i=0;i<nprocs;i++) {
     gp[i].rel_num_x = (int *) malloc(numlev*sizeof(int));;
     gp[i].rel_num_y = (int *) malloc(numlev*sizeof(int));;
     gp[i].eist = (int *) malloc(numlev*sizeof(int));;
     gp[i].ejst = (int *) malloc(numlev*sizeof(int));;
     gp[i].oist = (int *) malloc(numlev*sizeof(int));;
     gp[i].ojst = (int *) malloc(numlev*sizeof(int));;
     gp[i].rlist = (int *) malloc(numlev*sizeof(int));;
     gp[i].rljst = (int *) malloc(numlev*sizeof(int));;
     gp[i].rlien = (int *) malloc(numlev*sizeof(int));;
     gp[i].rljen = (int *) malloc(numlev*sizeof(int));;
     gp[i].multi_time = 0;
     gp[i].total_time = 0;
   }
   ...
   sesc_lock_init(&(locks->idlock));

   sesc_lock_init(&(locks->psiailock));

   sesc_lock_init(&(locks->psibilock));

   sesc_lock_init(&(locks->donelock));

   sesc_lock_init(&(locks->error_lock));

   sesc_lock_init(&(locks->bar_lock));


   sesc_barrier_init(&(bars->iteration));

   sesc_barrier_init(&(bars->gsudn));

   sesc_barrier_init(&(bars->p_setup));
 
   sesc_barrier_init(&(bars->p_redph));
 
   sesc_barrier_init(&(bars->p_soln));
 
   sesc_barrier_init(&(bars->p_subph));
 
   sesc_barrier_init(&(bars->sl_prini));

   sesc_barrier_init(&(bars->sl_psini));

   sesc_barrier_init(&(bars->sl_onetime));

   sesc_barrier_init(&(bars->sl_phase_1));

   sesc_barrier_init(&(bars->sl_phase_2));

   sesc_barrier_init(&(bars->sl_phase_3));

   sesc_barrier_init(&(bars->sl_phase_4));

   sesc_barrier_init(&(bars->sl_phase_5));

   sesc_barrier_init(&(bars->sl_phase_6));

   sesc_barrier_init(&(bars->sl_phase_7));

   sesc_barrier_init(&(bars->sl_phase_8));

   sesc_barrier_init(&(bars->sl_phase_9));

   sesc_barrier_init(&(bars->sl_phase_10));

   sesc_barrier_init(&(bars->error_barrier));
...
/* initialize constants and variables
  ...
  computeend = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
}
