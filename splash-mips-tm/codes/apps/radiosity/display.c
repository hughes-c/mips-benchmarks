...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(elem->ev1->ev_lock->lock));
#endif
	sesc_lock(&(elem->ev1->ev_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(elem->ev1->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(elem->ev1->ev_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(elem->ev1->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(elem->ev2->ev_lock->lock));
#endif
	sesc_lock(&(elem->ev2->ev_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(elem->ev2->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(elem->ev2->ev_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(elem->ev2->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(elem->ev3->ev_lock->lock));
#endif
	sesc_lock(&(elem->ev3->ev_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(elem->ev3->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(elem->ev3->ev_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(elem->ev3->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(ev->ev_lock->lock));
#endif
	sesc_lock(&(ev->ev_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(ev->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    ev->weight += weight ;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(ev->ev_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(ev->ev_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
