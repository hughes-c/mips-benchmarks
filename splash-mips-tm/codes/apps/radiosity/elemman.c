...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->cost_sum_lock));
#endif
	sesc_lock(&(global->cost_sum_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->cost_sum_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->cost_sum_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->cost_sum_lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(elem->elem_lock->lock));
#endif
	sesc_lock(&(elem->elem_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(elem->elem_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(elem->elem_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(elem->elem_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(pc->cost_lock->lock));
#endif
	sesc_lock(&(pc->cost_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(pc->cost_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(pc->cost_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(pc->cost_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(e->elem_lock->lock));
#endif
	sesc_lock(&(e->elem_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(e->elem_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( ! _LEAF_ELEMENT(e) )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(e->elem_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(e->elem_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->avg_radiosity_lock));
#endif
	sesc_lock(&(global->avg_radiosity_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->avg_radiosity_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    global->total_patch_area += ec->area ;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->avg_radiosity_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->avg_radiosity_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->free_element_lock));
#endif
	sesc_lock(&(global->free_element_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->free_element_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( global->free_element == 0 )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->free_element_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->free_element_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
    ...
    sesc_lock_init(&(global->free_element_lock));
;
}
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->free_interaction_lock));
#endif
	sesc_lock(&(global->free_interaction_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->free_interaction_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( global->free_interaction == 0 )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->free_interaction_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->free_interaction_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
    ...
    sesc_lock_init(&(global->free_interaction_lock));
;
}
