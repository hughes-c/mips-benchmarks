...
*/
  ...
  time_rad_start  = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
...
;
  ...
  time_rad_end  = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
                    ...
                    if( init_ray_tasks(0) )
                        {
                            {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(global->barrier), n_processors);
#endif
	sesc_barrier(&(global->barrier), n_processors);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(global->barrier));
#endif
};
                            process_tasks(0) ;
                        }
...
void radiosity()
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->index_lock));
#endif
	sesc_lock(&(global->index_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->index_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    process_id = global->index++;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->index_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->index_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  rad_start = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  refine_done = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  vertex_start = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  vertex_done = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  unsigned process_id;
{
    ...
    global->index = 1;  /* ****** */
    global->bsp_root = 0 ;
    sesc_lock_init(&(global->index_lock));
;
    sesc_lock_init(&(global->bsp_tree_lock));
;
    
    /* Initialize radiosity statistics variables */
    sesc_lock_init(&(global->avg_radiosity_lock));
;
    global->converged = 0 ;
    ...
    sesc_lock_init(&(global->cost_sum_lock));
;
    global->cost_sum = 0 ;
    global->cost_estimate_sum = 0 ;
    
    /* Initialize the barrier */
    sesc_barrier_init(&(global->barrier));
;
    sesc_lock_init(&(global->pbar_lock));
;
    global->pbar_count = 0 ;
    
    /* Initialize task counter */
    global->task_counter = 0 ;
    sesc_lock_init(&(global->task_counter_lock));
;
