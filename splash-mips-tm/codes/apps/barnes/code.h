...
global struct CellLockType {
    slock_t CL[MAXLOCK];        /* locks on the cells*/
} *CellLock;
...
struct GlobalMemory  {	/* all this info is for the whole system */
    ...
    sbarrier_t (Barstart);   /* barrier at the beginning of stepsystem  */
    sbarrier_t (Bartree);    /* barrier after loading the tree          */
    sbarrier_t (Barcom);     /* barrier after computing the c. of m.    */
    sbarrier_t (Barload);    
    sbarrier_t (Baraccel);   /* barrier after accel and before output   */
    sbarrier_t (Barpos);     /* barrier after computing the new pos     */
    slock_t (CountLock); /* Lock on the shared variables            */
    slock_t (NcellLock); /* Lock on the counter of array of cells for loadtree */
    slock_t (NleafLock);/* Lock on the counter of array of leaves for loadtree */
    slock_t (io_lock);
