  ...
  unsigned ProcID;
{
    ...
    {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(gl->PotengBar), NumProcs);
#endif
	sesc_barrier(&(gl->PotengBar), NumProcs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(gl->PotengBar));
#endif
};
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(gl->PotengSumLock));
#endif
	sesc_lock(&(gl->PotengSumLock));

#ifdef SIMICS
       SIMICS_LOCKED(&(gl->PotengSumLock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    *PTRF = *PTRF + LPTRF;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(gl->PotengSumLock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(gl->PotengSumLock));
       SIMICS_STOP_IGNORE();
#endif
