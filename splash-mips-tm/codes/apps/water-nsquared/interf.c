...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(gl->InterfVirLock));
#endif
	sesc_lock(&(gl->InterfVirLock));

#ifdef SIMICS
       SIMICS_LOCKED(&(gl->InterfVirLock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    *VIR = *VIR + LVIR;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(gl->InterfVirLock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(gl->InterfVirLock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(gl->MolLock[mol % MAXLCKS]));
#endif
	sesc_lock(&(gl->MolLock[mol % MAXLCKS]));

#ifdef SIMICS
       SIMICS_LOCKED(&(gl->MolLock[mol % MAXLCKS]));
       SIMICS_STOP_IGNORE();
#endif
#endif
}; 
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(gl->MolLock[mol % MAXLCKS]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(gl->MolLock[mol % MAXLCKS]));
       SIMICS_STOP_IGNORE();
#endif
    ...
    {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(gl->InterfBar), NumProcs);
#endif
	sesc_barrier(&(gl->InterfBar), NumProcs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(gl->InterfBar));
#endif
};
