...
InitOneFreeList(p)
{
  ...
  sesc_lock_init(&(mem_pool[p].memoryLock));
;
  if (p > 0) {
...
int size, home;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(mem_pool[home].memoryLock));
#endif
	sesc_lock(&(mem_pool[home].memoryLock));

#ifdef SIMICS
       SIMICS_LOCKED(&(mem_pool[home].memoryLock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
	...
	mem_pool[home].freeBlock[bucket] = NEXTFREE(result);
      {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(mem_pool[home].memoryLock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(mem_pool[home].memoryLock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  if (result) {
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(Global->memLock));
#endif
	sesc_lock(&(Global->memLock));

#ifdef SIMICS
       SIMICS_LOCKED(&(Global->memLock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    mem_pool[home].touched++;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(Global->memLock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(Global->memLock));
       SIMICS_STOP_IGNORE();
#endif
