...
ConstructGrid (int my_id, time_info *local_time, int time_all)
{
  ...
  init = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->single_lock));
#endif
	sesc_lock(&(G_Memory->single_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->single_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
	 ...
	 success = FALSE;
      {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->single_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->single_lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[pb->particle_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[pb->particle_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[pb->particle_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
	 ...
	 success = FALSE;
      {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[pb->particle_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[pb->particle_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
