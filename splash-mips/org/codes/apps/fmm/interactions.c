...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[b->exp_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[b->exp_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[b->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
      b->interaction_synch = 1;
      {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[b->exp_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[b->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[b->parent->exp_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[b->parent->exp_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[b->parent->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
      b->parent->interaction_synch += 1;
      {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[b->parent->exp_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[b->parent->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
...
ShiftMPExp (int my_id, box *cb, box *pb)
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[pb->exp_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[pb->exp_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[pb->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
   ...
   }
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[pb->exp_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[pb->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
...
XListInteraction (int my_id, box *source_box, box *dest_box)
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[dest_box->exp_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[dest_box->exp_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[dest_box->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
   ...
   }
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[dest_box->exp_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[dest_box->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
...
ShiftLocalExp (int my_id, box *pb, box *cb)
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->lock_array[cb->exp_lock_index]));
#endif
	sesc_lock(&(G_Memory->lock_array[cb->exp_lock_index]));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->lock_array[cb->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
   ...
   }
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->lock_array[cb->exp_lock_index]));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->lock_array[cb->exp_lock_index]));
       SIMICS_STOP_IGNORE();
#endif
