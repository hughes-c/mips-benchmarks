...
main (int argc, char *argv[])
{
  ...
  starttime = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
   ...
   for (i = 1; i < Number_Of_Processors; i++) {
	...
	sesc_spawn(ParallelExecute, NULL, 0);
;
   }
   ParallelExecute();
   {
...
ParallelExecute ()
{
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(G_Memory->synch), Number_Of_Processors);
#endif
	sesc_barrier(&(G_Memory->synch), Number_Of_Processors);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(G_Memory->synch));
#endif
};
   {
#ifdef ATOMIC
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->count_lock));
#endif
	sesc_lock(&(G_Memory->count_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->count_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
     ...
     G_Memory->id++;
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->count_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->count_lock));
       SIMICS_STOP_IGNORE();
#endif
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
  ...
  local_init_done = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
        }
      } 
  ...
  finish = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  endtime = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
   } 
   {
#ifdef SIMICS
...
StepSimulation (int my_id, time_info *local_time, int time_all)
{
  ...
  upward_end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  interaction_end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(G_Memory->synch), Number_Of_Processors);
  ...
  barrier_end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  ...
  downward_end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
