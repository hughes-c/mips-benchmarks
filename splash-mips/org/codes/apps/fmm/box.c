...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->mal_lock));
#endif
	sesc_lock(&(G_Memory->mal_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->mal_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
...
*/

   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->mal_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->mal_lock));
       SIMICS_STOP_IGNORE();
#endif
...
PrintBox (box *b)
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(G_Memory->io_lock));
#endif
	sesc_lock(&(G_Memory->io_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(G_Memory->io_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
      ...
      printf("Box has not been initialized yet.\n\n");
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(G_Memory->io_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(G_Memory->io_lock));
       SIMICS_STOP_IGNORE();
#endif
