...
/*
 * NAME
 *	ConvertPrimRayJobToRayMsg - convert primary ray job to the ray message 
 *
 * SYNOPSIS
 ...
 *
 * DESCRIPTION
 *	This routine converts the primary ray job to the ray message .
...
REAL	x, y;
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(gm->ridlock));
#endif
	sesc_lock(&(gm->ridlock));

#ifdef SIMICS
       SIMICS_LOCKED(&(gm->ridlock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
	ray->id = gm->rid++;
	{
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(gm->ridlock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(gm->ridlock));
       SIMICS_STOP_IGNORE();
#endif
...
INT	pid;
	...
	while (GetJobs(&job, pid) != WPS_EMPTY)
		...
		while (GetRayJobFromBundle(&job, &x, &y))
			{
			/* Convert the ray job to the ray message . */
