...
/* set values of temparray{1,3} to psim{1,3}  */
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_1), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_1), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_1));
#endif
}
...
/* to the values of the work1{1,2} arrays obtained from the
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_2), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_2), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_2));
#endif
}
...
/* put the laplacian of the work7{1,2} arrays in the work4{1,2}
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_3), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_3), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_3));
#endif
}
...
/* put the laplacian of the work4{1,2} arrays in the work7{1,2}
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_4), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_4), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_4));
#endif
}
...
/*     *******************************************************
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_5), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_5), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_5));
#endif
}
...
/*     *******************************************************
  ...
  multi_start = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
   }
  ...
  multi_end = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
...
/*  copy the solution for use as initial guess in next time-step  */
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_6), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_6), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_6));
#endif
}
...
/* after computing its private sum, every process adds that to the
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(locks->psibilock));
#endif
	sesc_lock(&(locks->psibilock));

#ifdef SIMICS
       SIMICS_LOCKED(&(locks->psibilock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
   global->psiai = global->psiai + psiaipriv;
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(locks->psibilock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(locks->psibilock));
       SIMICS_STOP_IGNORE();
#endif
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_7), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_7), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_7));
#endif
}
...
/*      *******************************************************
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_8), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_8), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_8));
#endif
}
...
/*      *******************************************************
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_9), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_9), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_9));
#endif
}
...
/* update the psi{1,3} matrices by adding 2*dtau*work3 to each */
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(bars->sl_phase_10), nprocs);
#endif
	sesc_barrier(&(bars->sl_phase_10), nprocs);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(bars->sl_phase_10));
#endif
}   
}
