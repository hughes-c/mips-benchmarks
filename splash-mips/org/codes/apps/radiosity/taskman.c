  ...
  unsigned process_id;
{
 ...
 retry_entry:
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->pbar_lock));
#endif
	sesc_lock(&(global->pbar_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->pbar_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    global->pbar_count++ ;
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->pbar_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->pbar_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  int  mode ;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(tq->q_lock));
#endif
	sesc_lock(&(tq->q_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(tq->q_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    /* Unlock the task queue */
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(tq->q_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(tq->q_lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(tq->f_lock));
#endif
	sesc_lock(&(tq->f_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(tq->f_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(tq->f_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(tq->f_lock));
       SIMICS_STOP_IGNORE();
#endif
    ...
    for( qid = 0 ; qid < n_taskqueues ; qid++ )
        {
            ...
            /* Initialize locks */
            sesc_lock_init(&(global->task_queue[ qid ].q_lock));
;
            sesc_lock_init(&(global->task_queue[ qid ].f_lock));
;
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->task_counter_lock));
#endif
	sesc_lock(&(global->task_counter_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->task_counter_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    
    {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->task_counter_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->task_counter_lock));
       SIMICS_STOP_IGNORE();
#endif
