  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->bsp_tree_lock));
#endif
	sesc_lock(&(global->bsp_tree_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->bsp_tree_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( parent == 0 )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->bsp_tree_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->bsp_tree_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->free_patch_lock));
#endif
	sesc_lock(&(global->free_patch_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->free_patch_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( global->free_patch == 0 )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->free_patch_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->free_patch_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
    ...
    sesc_lock_init(&(global->free_patch_lock));
 ;
