...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->free_elemvertex_lock));
#endif
	sesc_lock(&(global->free_elemvertex_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->free_elemvertex_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->free_elemvertex_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->free_elemvertex_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
    ...
    sesc_lock_init(&(global->free_elemvertex_lock));
;
  ...
  unsigned process_id;
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(e->edge_lock->lock));
#endif
	sesc_lock(&(e->edge_lock->lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(e->edge_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    if( ! _LEAF_EDGE(e) )
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(e->edge_lock->lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(e->edge_lock->lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->free_edge_lock));
#endif
	sesc_lock(&(global->free_edge_lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->free_edge_lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->free_edge_lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->free_edge_lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  unsigned process_id;
{
    ...
    /* Initialize global free list */
    sesc_lock_init(&(global->free_edge_lock));
;
    global->free_edge = 0 ;
  ...
  unsigned process_id;
{
    ...
    for( i = 0 ; i < MAX_SHARED_LOCK ; i++ )
        {
            sesc_lock_init(&(global->sh_lock[i].lock));
;
        }
