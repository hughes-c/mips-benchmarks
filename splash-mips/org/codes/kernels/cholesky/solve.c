...
char *argv[];
{
  ...
  sesc_lock_init(&(Global->waitLock));

  sesc_lock_init(&(Global->memLock));
...
void Go()
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(Global->waitLock));
#endif
	sesc_lock(&(Global->waitLock));

#ifdef SIMICS
       SIMICS_LOCKED(&(Global->waitLock));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
    ...
    gp->pid++;
  {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(Global->waitLock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(Global->waitLock));
       SIMICS_STOP_IGNORE();
#endif
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
  ...
  lc->rs = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
  }
  ...
  lc->rf = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
