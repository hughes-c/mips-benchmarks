...
struct global_memory {
   ...
   slock_t (lock_Index);                    /* for fetch and add to get ID */
   slock_t (rank_lock);                     /* for fetch and add to get ID */
   slock_t section_lock[MAX_PROCESSORS];  /* key locks */
   sbarrier_t (barrier_rank);                   /* for ranking process */
   sbarrier_t (barrier_key);                    /* for key sorting process */
   double *ranktime;
   double *sorttime;
   double *totaltime;
   int final;
...

{
  ...
  start = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
   ...
   sesc_lock_init(&(global->lock_Index));

   sesc_lock_init(&(global->rank_lock));

   {int i;for(i=0;i<MAX_PROCESSORS;i++){sesc_lock_init(&(global->section_lock[i]));
;}}
   sesc_barrier_init(&(global->barrier_rank));

   sesc_barrier_init(&(global->barrier_key));
  ...
  for (p_dummy = 0; (p_dummy < 1); p_dummy++)
    global->prefix_tree[i].done[p_dummy].flag = 0;
  {
  ...
  for (mon_dum1 = 0; mon_dum1 < 1; mon_dum1++)
    for (mon_dum2 = 0; mon_dum2 < 2; mon_dum2++) {
      global->prefix_tree[i].done[mon_dum1].count[mon_dum2] = 0;
      
{
	sesc_sema_init(&(global->prefix_tree[i].done[mon_dum1].queue[mon_dum2]),0);
}
    } 

  for (mon_dum1 = 0; mon_dum1 < 1; mon_dum1++) 
    sesc_lock_init(&(global->prefix_tree[i].done[mon_dum1].lock));
...
void slave_sort()
{
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->lock_Index));
#endif
	sesc_lock(&(global->lock_Index));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->lock_Index));
       SIMICS_STOP_IGNORE();
#endif
#endif
}
     ...
     global->Index++;
   {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->lock_Index));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->lock_Index));
       SIMICS_STOP_IGNORE();
#endif
...
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   ...
   {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(global->barrier_key), number_of_processors);
#endif
	sesc_barrier(&(global->barrier_key), number_of_processors);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(global->barrier_key));
#endif
} 
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
  ...
  time1 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
}
   }
...
/* generate histograms based on one digit */
  ...
  time2 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
}
     }
     ...
     {
#ifdef SIMICS
       SIMICS_ENTER_BARRIER(&(global->barrier_rank), number_of_processors);
#endif
	sesc_barrier(&(global->barrier_rank), number_of_processors);

#ifdef SIMICS
       SIMICS_LEAVE_BARRIER(&(global->barrier_rank));
#endif
}  
  ...
  lockAddr = (int *)&(global->prefix_tree[base + (offset >> 1)].done[0].lock);
  {;}
  {
#ifdef ATOMIC
        sesc_start_transaction();
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(global->prefix_tree[base + (offset >> 1)].done[0].lock));
#endif
	sesc_lock(&(global->prefix_tree[base + (offset >> 1)].done[0].lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(global->prefix_tree[base + (offset >> 1)].done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
  global->prefix_tree[base + (offset >> 1)].done[0].flag = 1;
  
{
  if (global->prefix_tree[base + (offset >> 1)].done[0].count[0]) {
    (global->prefix_tree[base + (offset >> 1)].done[0].count[0])--;
    
{
	sesc_vsema(&(global->prefix_tree[base + (offset >> 1)].done[0].queue[0]));
}   
  } else
    {
#ifdef ATOMIC
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(global->prefix_tree[base + (offset >> 1)].done[0].lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(global->prefix_tree[base + (offset >> 1)].done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(n->done[0].lock));
#endif
	sesc_lock(&(n->done[0].lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(n->done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
    ...
    
{
  n->done[0].count[0]++;
  {
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(n->done[0].lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(n->done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
  ...
  
{
  ...
  lockAddr = (int *)&(my_node->done[0].lock);
  {;}
  {
#ifdef ATOMIC
        sesc_start_transaction();
...
#ifdef SIMICS
       SIMICS_START_IGNORE();
       SIMICS_BEGIN_LOCK(&(my_node->done[0].lock));
#endif
	sesc_lock(&(my_node->done[0].lock));

#ifdef SIMICS
       SIMICS_LOCKED(&(my_node->done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
#endif
};
  if (!my_node->done[0].flag) {
    
{
  my_node->done[0].count[0]++;
  {
#ifdef ATOMIC
        sesc_commit_transaction();
#else
       ...
       SIMICS_START_IGNORE();
#endif
	sesc_unlock(&(my_node->done[0].lock));

#ifdef SIMICS
       SIMICS_UNLOCK(&(my_node->done[0].lock));
       SIMICS_STOP_IGNORE();
#endif
...
{
  sesc_psema(&(my_node->done[0].queue[0]));
};   
}
  }
  
{
  if (my_node->done[0].count[0]) {
    (my_node->done[0].count[0])--;
    
{
	sesc_vsema(&(my_node->done[0].queue[0]));
}   
  } else
    {
#ifdef ATOMIC
  ...
  
{
  ...
  time3 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
     }
  ...
  time4 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
     }
  ...
  time5 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
};
     }
  ...
  time6 = (sc_tp.tv_sec & 0x7ff)*1000000 + sc_tp.tv_usec;
}
